...
class FriendRepository {
  ...
  FriendRepository({
   ...
   Future<void> create({required String me, required String user}) =>
  ...
  Future<List<Friend>> friends(String uid) async =>
    (await firebaseFirestore.collection('friends')
        .where('user', isEqualTo: uid).get()).docs.futureMap<Friend>((friendSnapshot) async {
        ...
        return friendMapper.fromFirebase(friendSnapshot.data())
            .copyWith(user: user);
    });
    ...
    Future<bool> isFriend({required String me, required String user}) async =>
        (await firebaseFirestore
          ...
          .where('friend', isEqualTo: firebaseFirestore.collection('users')
            .doc(user))
              .get()).size > 0;
}