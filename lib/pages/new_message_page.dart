...
import 'package:telegram_app/mixin/search_component.dart';                      // fa riferimento alla classe SearchComponentsMixin
...
import 'package:telegram_app/widgtes/side_header.dart';
import 'package:telegram_app/widgtes/user_widget.dart';
import 'package:telegram_app/router/app_router.gr.dart';

class NewMessagePage extends ConnectivityWidget with AutoRouteWrapper, SearchComponentsMixin {
  final User user;

  const NewMessagePage({Key? key, required this.user}) : super(key: key);
  ...
  PreferredSizeWidget _appBar(BuildContext context, {isSearching = false}) => AppBar(
    leading: BackButton(
  ...
  Widget _usersItems(BuildContext context, {required List<models.User> users}) =>
    // Mostro la la lista degli utenti con cui sto chattando
    ListView.builder(
  ...
  Widget _friendsBody(BuildContext context) => BlocBuilder<FriendsBloc, FriendsState>(
    builder: (context, state) {
  ...
  Widget _friendsItems(BuildContext context, {required List<Friend> friends}) {
      final groupedFriends = friends.fold<LinkedHashMap<String, List<Friend>>>(
        LinkedHashMap(),
          (map, friend) => map..putIfAbsent(friend.user!.firstLetter, () => []).add(friend),
      );

      return DefaultStickyHeaderController(
        child: CustomScrollView(
          slivers: groupedFriends.values.map(
            (friends) => SliverStickyHeader(
              overlapsContent: true,
              // header: SideHeader(friends.first.substring(0, 1)),
              header: SideHeader(friends.first.user!.firstLetter),
              sliver: SliverPadding(
                padding: const EdgeInsets.only(left: 60),
                sliver: SliverList(
                  delegate: SliverChildBuilderDelegate(
                    // (context, index) => Text(friends[index]),
    ...
    Widget _shimmedItems() => ShimmedList(child: UserWidget.shimmed());
    ...
    Widget _noUsersWidget(BuildContext context) => MyErrorWidget(
      icon: Icons.search,
      // title: AppLocalizations.of(context)?.label_no_users_found_msg ?? '',
